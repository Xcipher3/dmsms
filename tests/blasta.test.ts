...
describe("Blasta SMS API - End-to-End Tests", () => {
  describe("Send SMS", () => {
    it("should return success response with mock data", async () => {
      const response = await client.api.sendSms.$post({
        json: {
          msg: "Test message",
          numbers: "+1234567890",
  ...
  describe("Check DLR", () => {
    it("should return delivery status for existing message", async () => {
      const response = await client.api.getDlr.$post({
  ...
  describe("Generate Token", () => {
    it("should return mock access token", async () => {
      const response = await client.api.getToken.$post({
        json: {
          username: "testuser",
          password: "testpass",
        },
  ...
  describe("Opt Out", () => {
    it("should return success opt-out response", async () => {
      const response = await client.api.optOut.$post({
        json: {
          numbers: "+1234567890",
          category: "promotional",
  ...
  describe("Opt In", () => {
    it("should return success opt-in response", async () => {
      const response = await client.api.optIn.$post({
        json: {
          numbers: "+1234567890",
          category: "promotional",
          reason: "User requested",
  ...
  describe("List Opt Outs", () => {
    it("should return list of opt-outs", async () => {
      const response = await client.api.listOptOuts.$get();
